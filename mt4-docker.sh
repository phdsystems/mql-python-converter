...
cmd_status() {
    ...
    if sudo docker ps | grep -q $CONTAINER_NAME; then
        ...
        sudo docker exec $CONTAINER_NAME supervisorctl status 2>/dev/null || print_error "Cannot get service status"
    else
...
cmd_health() {
    ...
    echo ""
    sudo docker exec $CONTAINER_NAME /usr/local/bin/healthcheck.sh || print_error "Health check failed"
}
...
cmd_monitor() {
    ...
    watch -n 2 "sudo docker exec $CONTAINER_NAME supervisorctl status"
}
...
cmd_backup() {
    ...
    # Backup MT4 data
    sudo docker cp $CONTAINER_NAME:/home/developer/mt4/MQL4 $BACKUP_DIR/ 2>/dev/null || print_error "Cannot backup MQL4 directory"
    sudo docker cp $CONTAINER_NAME:/home/developer/mt4/config $BACKUP_DIR/ 2>/dev/null || print_error "Cannot backup config directory"
