...
,
    ...
    def calculate(self, data: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        # Calculate indicator values
        fast_ma = self.sma(self.close, fast_length) if ma_type == "SMA" else self.ema(self.close, fast_length)
        results["fast_ma"] = fast_ma
        slow_ma = self.sma(self.close, sself.low_length) if ma_type == "SMA" else self.ema(self.close, sself.low_length)
        results["slow_ma"] = slow_ma
        bullish_cross = self.crossover(fast_ma, sself.low_ma)
        results["bullish_cross"] = bullish_cross
        bearish_cross = self.crossunder(fast_ma, sself.low_ma)
        results["bearish_cross"] = bearish_cross
...
def example_usage():
    ...
    indicator = SimpleMaCross()
    # With custom parameters:
    # indicator = SimpleMaCross(fast_length=9, slow_length=21, ma_type=SMA)
